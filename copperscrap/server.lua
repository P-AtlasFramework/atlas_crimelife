...
RegisterNetEvent('atlas_crimelife:cps:requestStrip', function(groupKey, propCoords)
    ...
    if Pending[cid] then
        lib.notify({ source = src, description = 'Already on a job', type = 'error', duration = 3000 })
        return
    end
    ...
    if nowSec() - (PropCooldowns[key] or 0) < config.perPropCooldownSec then
        lib.notify({ source = src, description = 'This one\'s already been stripped', type = 'error', duration = 3500 })
        return
    end
    ...
    if not hasItem then
        lib.notify({ source = src, description = 'Need wire cutters', type = 'error', duration = 3500 })
        return
    end
...
RegisterNetEvent('atlas_crimelife:cps:complete', function(success)
    ...
    if success then
        ...
        lib.notify({ source = src, description = (#parts > 0 and table.concat(parts, ' / ') or 'No usable scrap') ..
        ...
        if math.random() < (config.tool.breakChance or 0) then
            ...
            lib.notify({ source = src, description = 'Got shocked — cutters fried', type = 'error', duration = 4500 })
        else
            lib.notify({ source = src, description = 'Got shocked — try again', type = 'error', duration = 4000 })
        end
...
RegisterNetEvent('atlas_crimelife:cps:fenceSell', function(itemName, qty)
    ...
    if qty <= 0 or qty > config.fence.batchMax then
        lib.notify({ source = src, description = ('Sell 1-%d at a time'):format(config.fence.batchMax), type = 'error', duration = 3500 })
        return
    end
    ...
    if #(pCoords - vec3(fc.x, fc.y, fc.z)) > 4.0 then
        lib.notify({ source = src, description = 'Get to the fence', type = 'error', duration = 3000 })
        return
    end

    -- Cooldown
    if nowSec() - (FenceCooldowns[cid] or 0) < config.fence.sellCooldownSec then
        lib.notify({ source = src, description = 'Hold up — already counted last batch', type = 'error', duration = 3500 })
        return
    end
    ...
    if not has then
        lib.notify({ source = src, description = ('Don\'t have %d %s'):format(qty, itemName), type = 'error', duration = 3500 })
        return
    end
    ...
    if not removed then
        lib.notify({ source = src, description = 'Failed to take the materials', type = 'error', duration = 3500 })
        return
    end
    ...
    lib.notify({ source = src, description = ('Sold %d %s for $%d marked'):format(qty, itemName, pay), type = 'success', duration = 4500 })
end)
